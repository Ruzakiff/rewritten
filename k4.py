        ...
        if decereal_data is not None and decereal_data['outcomes'] is not None:
            for outcome in decereal_data['outcomes']:
                ...
                if bookmaker in self.sharpbookkey:
                    ...
                    if opposite_key in self.bookmakers[bookmaker]: #ONLY ONE SHARPBOOK NEEDS TO HAVE OPPOSITE
                        ...
                        #if outcome_key and opposite_key not in self.results:#TODO ASDFAWEFAWIOEHFAW HERERRERERERERE, OPPOSITE KEY STORE
                        ...
                        logging.warning(f'opposite outcome does not exist in sharpbookdata {outcome_key}${opposite_key}')
                # If the bookmaker is not in sharpbookkey, compare the outcome with the relevant part of the outcome pair
                else: #compare to current results? take it as is
...
#             market.update_market_data(market_data, game_id, bookmaker_data['key'])
#             ...
...
async def fetch_and_update_market_data(session, game, market_manager, bookmakers, markets, api_keys, current_key_index=0):
    ...
    try:
        ...
        logging.info(f"Bookmakers for game {game['id']}: {bookmakers}")
         # Get the keys of the bookmakers for the game
        ...
        for market in markets:
            ...
            if not (sharpbookmaker_keys.intersection(game_bookmaker_keys) and other_bookmaker_keys.intersection(game_bookmaker_keys)):
                ...
                return
        # Check if the game has any bookmakers
        ...
        if fetched_results:  # Check if results is not empty
            ...
            for fetched_game_data in fetched_results:
                ...
                if fetched_game_data is None:
                    ...
                    continue
                # Filter out bookmakers that don't have data 
                # WE ASK FOR ALL REQUIRED BOOKMAKERS AT ONCE, SO THEORITICALLY IT SHOULD EITHER BE GOOD OR NOT MAYBE CAN RETURN HERE IF NOT
                ...
                # Add the fetched data to the queue
